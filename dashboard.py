...
<head>
  ...
  <style>
    :root { --bg:#f6f7f9; --panel:#fff; --text:#1d2430; --muted:#667085; --line:#d0d5dd; --ok:#067647; --warn:#b54708; --critical:#b42318; --blue:#2563eb; }
    * { box-sizing: border-box; }
    body { margin:0; background:var(--bg); color:var(--text); font:14px Inter,system-ui,-apple-system,"Segoe UI",sans-serif; }
    header { display:flex; justify-content:space-between; align-items:center; gap:12px; padding:18px 24px; background:var(--panel); border-bottom:1px solid var(--line); }
    h1 { margin:0; font-size:18px; }
    main { display:grid; grid-template-columns:minmax(320px,450px) minmax(0,1fr); gap:16px; padding:16px 24px 24px; }
    button,select { border:1px solid var(--line); background:var(--panel); border-radius:6px; min-height:34px; padding:0 11px; cursor:pointer; font:inherit; }
    ...
    .card,.panel { background:var(--panel); border:1px solid var(--line); border-radius:10px; box-shadow:0 1px 2px rgba(16,24,40,.06); overflow:hidden; }
    .card { padding:12px; }
    .metric { font-size:22px; font-weight:750; }
    .label { color:var(--muted); font-size:12px; margin-top:3px; }
    .runner-row { width:100%; display:grid; grid-template-columns:minmax(0,1fr) auto; gap:12px; padding:13px 14px; border:0; border-bottom:1px solid var(--line); border-radius:0; text-align:left; }
    ...
    .status,.pill { border-radius:999px; padding:3px 8px; font-size:12px; border:1px solid var(--line); white-space:nowrap; }
    ...
    .status.disabled,.pill.info { color:var(--blue); background:#eff8ff; border-color:#b2ddff; }
    .pill.critical { color:var(--critical); background:#fef3f2; border-color:#fecdca; }
    .detail-head,.alerts,.cache,.output { padding:12px 14px; border-bottom:1px solid var(--line); }
    .detail-head { display:flex; justify-content:space-between; gap:14px; align-items:flex-start; }
    .detail-title strong { display:block; font-size:16px; }
    .detail-title span { display:block; color:var(--muted); margin-top:3px; font-size:12px; }
    .pill { display:inline-flex; margin:5px 4px 0 0; }
    ...
    pre { margin:0; padding:14px; min-height:320px; max-height:60vh; overflow:auto; background:#101828; color:#e4e7ec; font:12px/1.5 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace; }
    .empty { padding:24px; color:var(--muted); }
    @media (max-width:980px) { header { align-items:flex-start; flex-direction:column; } main { grid-template-columns:1fr; padding:12px; } .summary { grid-template-columns:repeat(2,minmax(0,1fr)); } .detail-head { flex-direction:column; } }
  </style>
</head>
<body>
  <header><h1>Local GitHub Actions Runners</h1><div class="toolbar"><button id="refresh">Atualizar</button><button id="startAll" class="primary">Start all</button><button id="stopAll" class="danger">Stop all</button></div></header>
  <main>
    <section>
      <div class="summary"><div class="card"><div class="metric" id="metricRunning">0</div><div class="label">rodando</div></div><div class="card"><div class="metric" id="metricStopped">0</div><div class="label">parados</div></div><div class="card"><div class="metric" id="metricAlerts">0</div><div class="label">alertas</div></div><div class="card"><div class="metric" id="metricDisk">-</div><div class="label">disco livre</div></div></div>
      <div class="panel" id="runnerList"></div>
    </section>
    <section class="panel">
      ...
      <div class="alerts"><strong>Alertas</strong><div id="alertOutput"></div></div>
      <div class="cache"><strong>Cache local</strong><div id="cacheOutput"></div></div>
      <div class="output" id="commandOutput"></div>
      <pre id="logOutput">Selecione um runner para ver o log.</pre>
    </section>
  </main>
<script>
let state={runners:[],summary:{},alerts:[],cache:[]}; let selected=null; let busy=false;
...
function renderSummary(){ $('metricRunning').textContent=state.summary.running||0; $('metricStopped').textContent=state.summary.stopped||0; $('metricAlerts').textContent=state.alerts?.length||0; $('metricDisk').textContent=state.summary.diskFreePercent!=null?`${state.summary.diskFreePercent}%`:'-';}
function renderList(){const list=$('runnerList'); list.innerHTML=''; if(!state.runners?.length){list.innerHTML='<div class="empty">Nenhum runner em runners.conf.</div>'; return;} for(const runner of state.runners){const status=runner.enabled===false?'disabled':(runner.running?'running':'stopped'); const row=document.createElement('button'); row.className=`runner-row ${selected===runner.name?'active':''}`; row.type='button'; row.onclick=()=>selectRunner(runner.name); row.innerHTML=`<div><div class="runner-name"></div><div class="runner-meta"></div><div class="runner-path"></div></div><span class="status ${status}">${status==='running'?'rodando':status==='disabled'?'desabilitado':'parado'}</span>`; row.querySelector('.runner-name').textContent=runner.name; row.querySelector('.runner-meta').textContent=`${runner.profile||'generic'} · ${runner.repo||'sem repo'} · pid ${runner.pid||'-'}`; row.querySelector('.runner-path').textContent=runner.path; list.appendChild(row);}}
function renderSelected(){const runner=state.runners.find(i=>i.name===selected); const pills=$('selectedPills'); pills.innerHTML=''; if(!runner){$('selectedName').textContent='Nenhum runner selecionado'; $('selectedPath').textContent=''; $('logOutput').textContent='Selecione um runner para ver o log.'; return;} $('selectedName').textContent=`${runner.name} · ${runner.running?'rodando':runner.enabled===false?'desabilitado':'parado'}`; $('selectedPath').textContent=runner.path; pills.appendChild(pill(`profile: ${runner.profile||'generic'}`)); if(runner.repo) pills.appendChild(pill(`repo: ${runner.repo}`)); if(runner.uptimeSeconds) pills.appendChild(pill(`uptime: ${Math.round(runner.uptimeSeconds/60)} min`,'ok')); if(runner.recentError) pills.appendChild(pill('erro recente','warn'));}
function renderAlerts(){const out=$('alertOutput'); if(!state.alerts?.length){out.innerHTML='<div class="label">Nenhum alerta ativo.</div>'; return;} const ul=document.createElement('ul'); for(const a of state.alerts){const li=document.createElement('li'); li.appendChild(pill(a.severity,a.severity==='critical'?'critical':a.severity==='warning'?'warn':'info')); li.appendChild(document.createTextNode(` ${a.message}`)); ul.appendChild(li);} out.innerHTML=''; out.appendChild(ul);}
...
async function loadStatus(){state=await requestJson('/api/status'); if(!selected&&state.runners.length>0) selected=state.runners[0].name; if(selected&&!state.runners.some(i=>i.name===selected)) selected=state.runners[0]?.name||null; renderSummary(); renderList(); renderSelected(); renderAlerts(); renderCache(); if(selected) await loadLog();}
...
def disk_summary() -> dict[str, object]:
    ...
    return {"totalBytes": usage.total, "usedBytes": usage.used, "freeBytes": usage.free, "freePercent": free_percent, "freeHuman": human_bytes(usage.free)}
...
def summary(runners: list[dict[str, object]], disk: dict[str, object]) -> dict[str, object]:
    ...
    return {"total": len(runners), "enabled": len(enabled), "disabled": len(disabled), "running": len(running), "stopped": len(stopped), "diskFreePercent": disk["freePercent"], "diskFreeHuman": disk["freeHuman"]}
...
def run_runner_action(action: str, target: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run([str(RUNNERS_SH), action, target], cwd=str(BASE_DIR), text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=45, check=False)
...
def status_payload() -> dict[str, object]:
    ...
    return {"ok": True, "runners": runners, "cache": cache, "disk": disk, "alerts": alerts, "summary": summary(runners, disk)}
