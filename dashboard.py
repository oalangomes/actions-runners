...
def process_roles_by_path(path: str) -> dict[str, list[int]]:
    roles: dict[str, list[int]] = {"listener": [], "worker": [], "shell": []}
    proc_root = Path("/proc")
    ...
    for pid_dir in proc_root.iterdir():
        ...
        if "Runner.Listener" in cmdline:
            roles["listener"].append(pid)
        elif "Runner.Worker" in cmdline:
            roles["worker"].append(pid)
        elif re.search(r"(^|[\s/])run\.sh([\s]|$)", cmdline):
            roles["shell"].append(pid)
    for role in roles:
...
def read_runners() -> list[dict[str, object]]:
    ...
    for raw_line in CONFIG_PATH.read_text(encoding="utf-8-sig").splitlines():
        ...
        if not name or not path:
            continue
        pid_raw = read_pid(name)
        roles = process_roles_by_path(path)
        listener_pids = roles["listener"]
        worker_pids = roles["worker"]
        shell_pids = roles["shell"]
        recovered_pid = listener_pids[0] if listener_pids else (shell_pids[0] if shell_pids else None)
        active_pid = pid_raw if is_running(pid_raw) else recovered_pid
...
def build_alerts(runners: list[dict[str, object]], cache: list[dict[str, object]], disk: dict[str, object], system: dict[str, object]) -> list[dict[str, str]]:
    ...
    for runner in runners:
        ...
        if runner["duplicateListener"]:
            alerts.append({"severity": "critical", "message": f"{name} tem mais de um Runner.Listener"})
        if runner["orphanWorker"]:
            alerts.append({"severity": "critical", "message": f"{name} tem Runner.Worker sem Listener/supervisor"})
        if not runner["running"] and not runner["orphanWorker"]:
...
def group_summary(runners: list[dict[str, object]], alerts: list[dict[str, str]]) -> list[dict[str, object]]:
    ...
    for group in sorted({str(runner["group"]) for runner in runners}):
        ...
        result.append({"name": group, "total": len(members), "enabled": len(enabled), "running": len(running), "stopped": len(enabled) - len(running), "alerts": alert_count})
    return result
...
def build_recommendations(runners: list[dict[str, object]], groups: list[dict[str, object]], cache: list[dict[str, object]], system: dict[str, object]) -> list[dict[str, str]]:
    ...
    if any(bool(runner["duplicateListener"]) for runner in runners):
    ...
    if not recommendations:
        recommendations.append({"severity": "info", "message": "Central saudável; Listener + Worker é normal durante um job"})
...
def status_payload() -> dict[str, object]:
    ...
    return {"ok": True, "runners": runners, "groups": groups, "cache": cache, "disk": disk, "system": system, "alerts": alerts, "recommendations": build_recommendations(runners, groups, cache, system), "summary": summary(runners, disk, alerts)}
